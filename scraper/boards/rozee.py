...
class RozeeParser(BaseJobParser):
    ...
    board_name = "rozee"
    enrich_from_detail_pages = False
    ...
    def parse_listing(self, response: Any) -> List[str]:
        ...
        try:
            ...
            if not cards:
                cards = response.css("article.job")
            if not cards:
                cards = response.css("div.job-box")
            if not cards:
                # Listing markup sometimes wraps only the header row.
                cards = response.css("div.jhead")
            if not cards:
    ...
    def _parse_card(self, card: Any) -> Optional[JobData]:
        ...
        # Title + URL
        title_link = self._css_first(card, ["div.jhead h3 a", "h3.s-18 a", "h3 a"])
        if not title_link:
            return None
