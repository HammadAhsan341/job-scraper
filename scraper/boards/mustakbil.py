...
class MustakbilParser(BaseJobParser):
    ...
    def parse_job(self, response: Any) -> Optional[JobData]:
        ...
        try:
            ...
            if not description_text or len(description_text) < 50:
                print(f"Mustakbil: Description too short at {response.url}")
                return None
    ...
    def parse_listing(self, response: Any) -> List[str]:
        ...
        try:
            ...
            for link in links:
                title_text = self.clean_text(self._get_text(link)) or (getattr(link, "text", None) or "")
                title_text = self.clean_text(str(title_text))
                    ...
                    if href:
                        ...
                        if href not in urls:
                            ...
                            self._listing_jobs.append({
                                "job_id": self.generate_job_id(title_text or href, "Unknown", "Pakistan"),
                                "title": title_text or href.rsplit("/", 1)[-1],
                                "company": "Unknown",
                                "location": "Pakistan",
                                "job_url": href,
                                "board": self.board_name,
    ...
    def build_search_url(self, query: str, location: str = "", page: int = 1) -> str:
        ...
        import urllib.parse
        keywords = urllib.parse.quote(query or "")
