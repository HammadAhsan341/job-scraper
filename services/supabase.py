...
from services.job_record import job_to_record
...
class SupabaseService:
    ...
    def bulk_insert_jobs(self, jobs: List[Dict]) -> int:
        ...
        try:
            ...
            print(f"Inserted/updated {len(records)} jobs to Supabase")
            return len(records)
        except Exception as e:
            print(f"Failed to bulk insert jobs: {e}; retrying one-by-one")
            ok = 0
            for record in records:
                try:
                    ...
                    print(f"Failed to insert {record.get('job_id')}: {row_exc}")
            if ok:
                print(f"Inserted/updated {ok}/{len(records)} jobs to Supabase")
            return ok

    def clear_orphan_processed_jobs(self) -> int:
        """Remove processed_jobs IDs that no longer have a jobs row."""
        deleted = 0
        offset = 0
        try:
            while True:
                ...
                ids = [row["job_id"] for row in (response.data or []) if row.get("job_id")]
                if not ids:
                    break
                existing = self.get_jobs_by_ids(ids)
                orphans = [job_id for job_id in ids if job_id not in existing]
                if orphans:
                    self.client.table("processed_jobs").delete().in_("job_id", orphans).execute()
                    deleted += len(orphans)
                else:
                    offset += len(ids)
                if len(ids) < 500:
    ...
    def is_job_processed(self, job_id: str) -> bool:
        ...
        try:
            ...
            print(f"Failed to check processed job: {e}")
            return False

    def mark_job_processed(self, job_id: str) -> bool:
        """Record a processed job_id so parallel workers skip duplicates."""
        try:
            self.client.table("processed_jobs").upsert(
                {
                    "job_id": job_id,
    ...
    def log_scraping_error(self, url: str, error: str, retries: int = 0) -> bool:
        ...
        try:
            ...
            print(f"Failed to log scraping error: {e}")
            return False
